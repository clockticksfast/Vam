...
local library = {colorpicking = false;tabbuttons = {};tabs = {};options = {};flags = {};scrolling = false;multiZindex = 200;toInvis = {};libColor = Color3.fromRGB(128, 179, 255);disabledcolor = Color3.fromRGB(233, 0, 0);blacklisted = {Enum.KeyCode.W,Enum.KeyCode.A,Enum.KeyCode.S,Enum.KeyCode.D,Enum.UserInputType.MouseMovement}}
menu.bg.bg.bg.bg.tabbuttons.button.element.BackgroundColor3 = library.libColor
...
menu.bg.pre.Text = 'evc<font color="' .. toHex(library.libColor) .. '">.ac</font>'
...
function library:addTab(name)
    ...
    function tab:createGroup(pos,groupname) -- newTab[pos == 0 and "left" or "right"] 
        ...
        function group:addToggle(args)
            ...
            function toggle:addKeybind(args)
                ...
                inputService.InputBegan:Connect(function(key)
                    key = key.KeyCode == Enum.KeyCode.Unknown and key.UserInputType or key.KeyCode
                    if next then
            ...
            function toggle:addColorpicker(args)
                ...
                front.Name = "front"
                front.Parent = mid
                front.BackgroundColor3 = Color3.fromRGB(103, 89, 179)
                front.BorderColor3 = Color3.fromRGB(0, 0, 0)
				...
				colorFrame_2.BorderColor3 = Color3.fromRGB(40, 40, 40)
                ...
                hueframe.BorderColor3 = Color3.fromRGB(40, 40, 40)
                ...
                picker.Name = "picker"
                picker.Parent = main
                picker.BackgroundColor3 = args.color or library.libColor
                ...
                pickerframe.BorderColor3 = Color3.fromRGB(40, 40, 40)
                ...
                hue.Name = "hue"
                hue.Parent = main_2
                hue.BackgroundColor3 = args.color or library.libColor
                ...
                clr.BackgroundColor3 = Color3.fromRGB(20, 20, 20)
                clr.BackgroundTransparency = 1.000
                clr.BorderColor3 = Color3.fromRGB(40, 40, 40)
        ...
        function group:addSlider(args, sub)
            ...
            fill.BorderColor3 = Color3.fromRGB(40, 40, 40)
            ...
            local function updateValue(value)
                if library.colorpicking then return end
                value = roundValue(value, args.rounding or 0) -- Rounding to 2 decimal places
                if value ~= 0 then
                ...
                valuetext.Text = value .. sub -- Displaying rounded value
                library.flags[args.flag] = value
                if args.callback then
                    args.callback(value)
                end
        ...
        function group:addList(args)
            if not args.flag or not args.values or not args.text then return warn("⚠️ incorrect arguments ⚠️") end
            groupbox.Size = groupbox.Size + UDim2.new(0, 0, 0, 35)
...
--args.multiselect and "..." or ""
            ...
            button.MouseButton1Click:Connect(function()
                if not library.colorpicking then
                    ...
                    dumbtriangle.Rotation = dumbtriangle.Rotation + 180
                end
            end)
            button.MouseEnter:Connect(function()
                main.BorderColor3 = library.libColor
...
if not isfolder('Evc') then
    makefolder('Evc')
end
if not isfolder('Evc\\The final stand 2') then
    makefolder('Evc\\The final stand 2')
end
function library:refreshConfigs()
    local tbl = {}
    for i, v in next, listfiles("Evc\\The final stand 2") do
        local match = v:match("The final stand 2\\\\(.+)")
        if match then
            table.insert(tbl, match)
        end
    end
    library.options["ConfigBox"].refresh(tbl)
end
...
function library:createConfig()
    ...
    for i,v in next, library.flags do
        if not library.options[i].skipflag then
    ...
    writefile("Evc\\The final stand 2\\"..name..".cfg", game:GetService("HttpService"):JSONEncode(jig))
    library:refreshConfigs()
end

function library:loadConfig()
    local name = library.flags["ConfigBox"]
    local config = game:GetService("HttpService"):JSONDecode(readfile("Evc\\The final stand 2\\"..name))
...
function library:deleteConfig()
    local File = "Evc\\The final stand 2\\"..library.flags["ConfigBox"]
    delfile(File)
    library:refreshConfigs()
end
return library
